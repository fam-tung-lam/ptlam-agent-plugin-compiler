...
import {
  ...
  useSchemaVersion2,
  useSkillDependencyDepthViolation,
...
describe("AgentPluginCompiler", () => {
  ...
  it("rejects depth violations before generated reads or writes", async () => {
    // GIVEN: Managed output exists before the authored graph reaches depth three.
    const rootDir = await createCompilerRepository();
    await useSkillDependencyGraph(rootDir);
    const compiler = codexCompiler(rootDir);
    await compiler.compile();
    const catalogPath = path.join(rootDir, "skills", "README.md");
    await writeFile(catalogPath, "preserve managed bytes\n", "utf8");
    await useSkillDependencyDepthViolation(rootDir);
    const generatedRead = vi.spyOn(filesystem, "readGeneratedSnapshot");

    // WHEN: Every public validating operation evaluates the invalid repository.
    const operations = [
      () => compiler.validate(),
      () => compiler.check(),
      () => compiler.compile(),
    ...
    // THEN: Each fails through one validation channel before generated I/O.
    for (const operation of operations) {
      await assert.rejects(operation(), (error: unknown) => {
        assert.ok(error instanceof PluginValidationError);
        assert.match(
          error.message,
          /skill "alpha-skill" reaches configured dependency depth limit 3 through alpha-skill -> middle-skill -> shared-skill -> boundary-skill/u,
        );
        assert.match(
          error.message,
    ...
    assert.equal(generatedRead.mock.calls.length, 0);
    assert.equal(
      await readFile(catalogPath, "utf8"),
      "preserve managed bytes\n",
    );
  });

  it("keeps generated bytes unchanged for explicit null configuration", async () => {
    // GIVEN: A schema-v2 plugin has compiled with omitted configuration.
    const rootDir = await createCompilerRepository();
    await useSchemaVersion2(rootDir);
    const compiler = codexCompiler(rootDir);
    await compiler.compile();
    const catalogPath = path.join(rootDir, "skills", "README.md");
    const providerPath = path.join(rootDir, ".codex-plugin", "plugin.json");
    ...
    manifest.config = { skill_dependency_depth_limit: null };
    await writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`);

    // WHEN: Compilation repeats with the equivalent explicit unlimited value.
    const result = await compiler.compile();

    // THEN: No managed artifact changes and generated bytes stay identical.
    assert.deepEqual(result.writeResult.changedPaths, []);
    assert.deepEqual(
      await Promise.all([readFile(catalogPath), readFile(providerPath)]),
      bytesBefore,
    );
  });
