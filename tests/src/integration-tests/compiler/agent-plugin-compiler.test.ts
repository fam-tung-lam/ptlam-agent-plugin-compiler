...
describe("AgentPluginCompiler", () => {
  it("uses one nested source mapping across validate, compile, and check", async () => {
    // GIVEN: Equivalent repositories author the same v1 skill at flat and grouped source paths.
    const flatRoot = await createCompilerRepository();
    const nestedRoot = await createCompilerRepository();
    ...
    await rename(
      path.join(nestedRoot, "plugin", "skills", "alpha-skill"),
      path.join(groupedParent, "alpha-skill"),
    );
    const flatCompiler = codexCompiler(flatRoot);
    const nestedCompiler = codexCompiler(nestedRoot);

    // WHEN: The API validates, compiles, and checks the grouped repository.
    const validation = await nestedCompiler.validate();
    const [flatCompilation, nestedCompilation] = await Promise.all([
      flatCompiler.compile(),
      nestedCompiler.compile(),
    ]);
    const check = await nestedCompiler.check();

    // THEN: The nested source path is retained while all generated bytes remain flat and identical.
    assert.equal(
      validation.plugin.skills[0]?.source_path,
      "plugin/skills/projects/health-connector/alpha-skill",
    );
    assert.equal(flatCompilation.verified, true);
    assert.equal(nestedCompilation.verified, true);
    ...
    for (const generatedPath of [
      ...
      assert.deepEqual(
        await readFile(path.join(nestedRoot, generatedPath)),
        await readFile(path.join(flatRoot, generatedPath)),
      );
    }
    await assert.rejects(lstat(path.join(nestedRoot, "skills", "projects")), {
      code: "ENOENT",
    });
  });
  ...
  it("rejects an unowned grouping file before applying managed writes", async () => {
    // GIVEN: A grouping directory contains a regular file outside every discovered skill root.
    const rootDir = await createCompilerRepository();
    const groupingRoot = path.join(rootDir, "plugin", "skills", "projects");
    await mkdir(groupingRoot);
    await writeFile(path.join(groupingRoot, "README.md"), "# Group notes\n");

    // WHEN: Compilation attempts to map the authored skill tree.
    const compilation = codexCompiler(rootDir).compile();

    // THEN: Discovery fails with the complete source path before any managed path is written.
    await assert.rejects(
      compilation,
