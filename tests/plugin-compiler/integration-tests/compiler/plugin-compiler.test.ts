...
import { PluginCompiler } from "../../../../src/compiler/index.ts";
import {
  createOutputState,
...
function codexCompiler(rootDir: string): PluginCompiler {
  return new PluginCompiler({ rootDir, providerIds: ["codex"] });
}
...
describe("PluginCompiler", () => {
  ...
  it("generates and verifies only the common tree and enabled provider", async () => {
    // GIVEN: A valid repository has stale selected outputs and disabled Claude bytes.
    const rootDir = await createCompilerRepository();
    const compiler = codexCompiler(rootDir);

    // WHEN: The same selected plan is generated twice and then checked.
  ...
  it("reports failed post-write verification from reread facts", async () => {
    ...
    // WHEN: Generation writes and then verifies the selected plan.
    const result = await codexCompiler(rootDir).generate();
  ...
  it("aggregates filesystem and core diagnostics before any write", async () => {
    ...
    // WHEN: Generation attempts to load and validate the authored snapshot.
    const generation = codexCompiler(rootDir).generate();
